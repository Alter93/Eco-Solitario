...
test('Enter starts and keyboard movement advances both player and camera',()=>{
 const b=boot();b.key('keydown','Enter');b.frames(1);for(const e of b.game.state.enemies)e.dead=true;const x=b.game.P.x;
...
test('automatic run has no sprint button and exhaustion recovers without oscillating',()=>{
 const b=boot();b.game.start();b.frames(1);for(const e of b.game.state.enemies)e.dead=true;
 assert.equal(b.buttons.run,undefined);b.key('keydown','d');b.frames(1);assert.equal(b.game.P.vx,285);
 b.game.P.stamina=0;b.frames(.5);assert.equal(b.game.P.vx,185);
 b.frames(2);assert.equal(b.game.P.vx,285);
});
test('full chapter traversal with movement and combat reaches tower without teleporting',()=>{
 const b=boot();b.game.start();b.key('keydown','d');b.key('keydown','k');
 for(let i=0;i<600&&!b.game.state.complete&&!b.game.state.gameOver;i++){
  const threat=b.game.state.enemies.some(e=>!e.dead&&e.x>b.game.P.x&&e.x-b.game.P.x<440);
  b.key(threat?'keydown':'keyup','j');
  if(b.game.P.onGround&&threat){b.key('keyup',' ');b.key('keydown',' ');}
  b.frames(.1);
 }
...
test('eight-pose run yields to weapons and jump, then returns without changing physics',()=>{
 const b=boot();const atlas={testAtlas:true};b.game.setRunAtlas(atlas);b.game.start();b.frames(1);for(const e of b.game.state.enemies)e.dead=true;
 b.key('keydown','d');b.frames(.2);const phases=new Set();
